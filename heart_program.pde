...
PShape s;
float BPM = 120; //her kan man vælge hvor mange BBP hjertet skal pumpe med (det er de store tryk den tælder, tror ikke de mindre skal tælles med)
...
void draw() {
  ...
  camera(width/2.0, height/2.0, (height/2.0)-300-pow((sin(radians(frameCount*12*BPM/60))+sin(radians(frameCount*12*BPM/60)-1.45)*sin(radians(frameCount*12*BPM/60))),10)*2.2/(BPM/60), width/2.0, height/2.0, 0, 0, 1, 0);
